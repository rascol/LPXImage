...
int main(int argc, char** argv) {
    ...
    // Now render the LPXImage back to standard image for verification
    std::cout << "Rendering image back to standard format for verification..." << std::endl;
    
    // Initialize renderer and load scan tables
    // lpx::LPXRenderer renderer;
    // renderer.setScanTables(serialLpxImage->getScanTables());

    // Get the scan tables from your multithreaded image
    auto scanTables = mtLpxImage->getScanTables();

    // Then set it to the renderer
    lpx::LPXRenderer renderer;
    ...
    if (renderedMtImage.empty()) {
        std::cerr << "Failed to render mt image" << std::endl;
        return -1;       
    }
    ...
    std::cout << "Saved rendered image to output_mt_rendered.jpg" << std::endl;
    ...
    std::cout << "Testing completed. Rendered image saved." << std::endl;
